...
class MailTemplates:
    ...
    def init_app(self, app, template_folder="emails", inline_css=True, auto_text_body_from_html=True, markdown_options=None):
        loader = RemoveFrontmatterLoader(FileSystemLoader(os.path.join(app.root_path, app.config.get("MAIL_TEMPLATES_FOLDER", template_folder))))
...
class TemplatedEmailMessage(EmailMultiAlternatives):
    def __init__(self, filename, to, **kwargs):
        ...
        relpath = current_app.jinja_env.get_template(filename).filename
...
def render_mail(filename, **ctx):
    ...
    frontmatter = get_template_frontmatter(state.jinja_env, filename)
    if frontmatter:
